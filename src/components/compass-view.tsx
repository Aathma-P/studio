...
export default function CompassView({ items }: CompassViewProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-between bg-gray-900 text-white p-4 overflow-hidden">
      <div className="text-center opacity-80">
        <p className="text-lg">Next Item</p>
        <p className="text-2xl font-bold">
            {sortedItems[0]?.name || "All items found"}
        </p>
      </div>
      ...
      >
        <div className="relative w-64 h-64 md:w-80 md:h-80 rounded-full bg-gray-800/50 border-4 border-gray-700 flex items-center justify-center">
            ...
            <ArrowUp 
                className={cn(
                    'w-32 h-32 md:w-40 md:h-40 text-green-400 transition-transform duration-500 ease-in-out',
                    getArrowRotation()
                )} 
                strokeWidth={1}
            />
        ...
        <div className="mt-8 text-center">
            ...
            {currentInstruction.distance && currentInstruction.type === 'straight' && (
                <p className="mt-2 text-xl md:text-2xl text-green-400 font-medium">
